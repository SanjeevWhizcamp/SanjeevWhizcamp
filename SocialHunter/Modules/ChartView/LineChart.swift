...
open class LineChart: UIView {
    ...
    */
    ...
    open var colors: [UIColor] = [
        UIColor(named: "DarkBlue")!.withAlphaComponent(1),
        UIColor(named: "LineGreen")!.withAlphaComponent(1),
        UIColor(named: "LineRed")!.withAlphaComponent(1),
...
//        delegate?.didSelectDataPoint(CGFloat(rounded), yValues: yValues)
    ...
    fileprivate func drawXLabels() {
        ...
        for (index, _) in xAxisData.enumerated() {
            ...
            label.font = UIFont.init(name: "Muli", size: 13)//preferredFont(forTextStyle: UIFont.TextStyle.caption2)
    ...
    fileprivate func drawYLabels() {
        ...
        for i in stride(from: start, through: stop, by: step){
            ...
            let label = UILabel(frame: CGRect(x: -12, y: yValue, width: y.axis.inset + 10, height: y.axis.inset))
            label.font = UIFont.init(name: "Muli", size: 13)
            label.textAlignment = .center
